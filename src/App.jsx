...
// [CITY] City system modules: shell, system picker, data store + cohort helpers.
import CityShell from './city/CityShell.jsx';
import SystemGate from './city/SystemGate.jsx';
import { useCityStore } from './city/cityStore.js';
import { systemsForStaff, canOpenAdminPanel, cohortRole, cohortAccessRevoked, defaultCohortForCreator, accessForSystem } from './city/cohort';
...
const DEFAULT_STAFF_ROLES = [
  ...
  { id: 'city_lead', label: 'City Lead' },   // [CITY]
  { id: 'coach', label: 'Coach' },           // [CITY]
];
const ROLE_LABEL = {
...
const STAFF_ADMIN_ROLES = ['academy_lead', 'afa_lead', 'curriculum_specialist', 'placement_ops', 'city_lead' /* [CITY] */, 'resource_planner'];
...
// [CITY] injectedAuth: identity supplied by a host (render tests / embedded
// usage). When present, this app skips Firebase auth entirely and uses it.

export default function App({ injectedAuth = null, onInjectedLogout = null }) {
  const [auth, setAuth] = useState(null);
  const [authLoaded, setAuthLoaded] = useState(Boolean(injectedAuth));
  const [authError, setAuthError] = useState('');
  const [authBusy, setAuthBusy] = useState(false);
  const [system, setSystem] = useState(null); // [CITY] staff Training System choice
  const cityStore = useCityStore();           // [CITY] City collections (roster, items, tasks, settings, types, modes)

  useEffect(() => {
    if (injectedAuth) return undefined; // [CITY] host-managed identity, no Firebase
    ...
    if (!firebaseAuth) { setAuthLoaded(true); return undefined; } // [CITY] missing/stubbed config must never blank-screen
  ...
  // [CITY] with a host-managed identity, logout is delegated to the host.
  const handleLogout = () => { if (injectedAuth) { if (onInjectedLogout) onInjectedLogout(); return undefined; } return signOut(firebaseAuth); };
  const [localTestAccounts, setLocalTestAccounts] = useState(null);
  useEffect(() => {
    // [DEV-ONLY] local test logins: fixtures live in
    // public/local-test-accounts.json (git-ignored, never shipped). Dev builds
    // fetch it over HTTP; production ignores it, and a fresh clone or CI has
    // no file to load, so the feature simply stays off everywhere else.
    if (!import.meta.env.DEV || injectedAuth) return undefined;
    let live = true;
    fetch(`${import.meta.env.BASE_URL}local-test-accounts.json`).then(res => {
      if (!res.ok) throw new Error('no local test accounts');
      return res.json();
    }).then(list => { if (live && Array.isArray(list)) setLocalTestAccounts(list); }).catch(() => { /* local test fixtures not present */ });
    return () => { live = false; };
  }, []);

  const handleLocalLogin = import.meta.env.DEV ? key => {
    const entry = (localTestAccounts || []).find(a => a.key === key);
    if (!entry?.user) return;
    localStorage.setItem('wa14-local-test-auth', JSON.stringify(entry.user));
    setAuth(entry.user);
  } : null;
  ...
  // [CITY] Training System routing: Fellows derive their system from their
  // cohort (Year 1/Year 2 → City, Winter Academy cohort → WA); staff with both
  // systems get the picker automatically; single-system staff go straight in.
  const activeAuth = injectedAuth || auth;
  const activeSystem = system || (activeAuth && activeAuth.role !== 'fellow' ? systemsForStaff(activeAuth)[0] : null);
  // Per-system access: a staffer entering City gets their City access, a
  // staffer entering the Winter Academy gets their waAccess (or 'full' when
  // they never carried access at all, e.g. legacy local fixtures).
  const systemAuth = activeAuth ? {
    ...activeAuth,
    access: activeAuth.access || (activeAuth.role === 'fellow' ? undefined : accessForSystem(activeAuth, activeSystem || 'winter_academy', STAFF_ADMIN_ROLES)) || 'full'
  } : null;
  const logoutAll = injectedAuth ? (onInjectedLogout || (() => { })) : handleLogoutAll;
  const staffSystems = systemAuth && systemAuth.role !== 'fellow' ? systemsForStaff(systemAuth) : [];
  if (!injectedAuth && !authLoaded) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '400px', color: '#9DB09D', fontFamily: FONT }}>Loading…</div>;
  if (!systemAuth) return <LoginGate onLogin={handleLogin} onLocalLogin={handleLocalLogin} onRedirectLogin={handleRedirectLogin} error={authError} busy={authBusy} localTestAccounts={localTestAccounts || []} />;
  if (systemAuth.role === 'fellow') {
    if (systemAuth.cohortRole === 'winter_academy') return <MainApp auth={systemAuth} onLogout={logoutAll} />;
    return <CityShell auth={systemAuth} system="city" setSystem={null} onLeave={logoutAll} store={cityStore} />;
  }
  if (staffSystems.length > 1 && !system) return <SystemGate auth={systemAuth} onPick={setSystem} onLogout={logoutAll} />;
  if ((system || staffSystems[0]) === 'city') return <CityShell auth={systemAuth} system={system} setSystem={setSystem} onLeave={logoutAll} store={cityStore} />;
  // Staff with both systems entered the Winter Academy: they can switch back
  // to the system picker from the TopBar (see MainApp onSwitchSystem).
  return <MainApp auth={systemAuth} onLogout={logoutAll} onSwitchSystem={staffSystems.length > 1 ? () => setSystem(null) : null} />;
}
...
async function resolveRole(user) {
  ...
  // [CITY] the shared staff table (wa14-planners, maintained by both the WA
  // Staff tab and the City admin panel) is the source of truth for staff
  // role/access/systems; wa14-accounts is kept only as a legacy fallback.
  let staff = [];
  try {
    const res = await storage.get('wa14-planners');
    staff = res && res.value ? JSON.parse(res.value) : [];
  ...
  } catch (e) { /* legacy fallback only */ }

  if (em === SUPERADMIN_EMAIL) return { ok: true, email: em, role: 'superadmin', access: 'full', systems: ['winter_academy', 'city'], adminPanel: true, name: user.displayName || 'Superadmin', uid: user.uid };

  if (STAFF_EMAIL_RE.test(em)) {
    const match = staff.find(a => String(a.email || '').toLowerCase() === em)
      || accounts.find(a => String(a.email || '').toLowerCase() === em);
    const role = match?.role || 'staff';
    if (role === 'superadmin') {
      // A staff record promoted to Superadmin by the built-in superadmin: same
      // powers, resolved from the staff table like everyone else.
      return { ok: true, email: em, role: 'superadmin', access: 'full', systems: ['winter_academy', 'city'], adminPanel: true, name: match?.name || user.displayName || 'Superadmin', uid: user.uid };
    }
    const access = accessForSystem({ ...match, role }, 'city', STAFF_ADMIN_ROLES);
    // [CITY] staff carry their Training Systems + admin-panel rights on the
    // auth object; both decide the post-login system picker.
    return { ok: true, email: em, role, access, systems: systemsForStaff({ role, systems: match?.systems }), adminPanel: canOpenAdminPanel({ role, access }), name: match?.name || user.displayName || em.split('@')[0], uid: user.uid };
  }
  ...
  if (FELLOW_EMAIL_RE.test(em)) {
    try {
      ...
      if (match) {
        // [CITY] Fellows carry their cohort + derived role; alumni (fellowship
        // ended after 31 Dec of cohort+1) are signed out with a clear message.
        if (cohortAccessRevoked(match.cohort)) {
          const endYear = Number(match.cohort) + 1;
          return { ok: false, error: 'Your fellowship access ended on 31 December ' + endYear + '. Your record is kept, but this account no longer opens the calendars.' };
        }
        return { ok: true, email: em, role: 'fellow', name: match.name, fellowId: match.id, afaGroup: match.afaGroup || '', cohort: match.cohort ?? null, cohortRole: cohortRole(match.cohort), uid: user.uid };
      }
    } catch (e) { return { ok: false, error: 'Could not verify right now -- please try again.' }; }
...
function LoginGate({ onLogin, onLocalLogin, onRedirectLogin, error, busy, localTestAccounts = [] }) {
  // [DEV-ONLY] the test-account picker. The list is fetched by App from the
  // git-ignored public/local-test-accounts.json (dev server only); without it
  // this box never renders, so fresh clones / CI see nothing.
  const [localKey, setLocalKey] = useState('superuser');
  return (
    <div className="wa14-app min-h-[480px] flex items-center justify-center bg-wa-bg" style={{ fontFamily: FONT }}>
      <div className="wa-card p-8 w-[340px] max-w-[88vw]">
        ...
        {import.meta.env.DEV && localTestAccounts.length > 0 && onLocalLogin && <div className="mt-[22px] pt-4 border-t border-wa-borderdeep"><div className="text-[11.5px] text-wa-muted mb-2">Local testing only. These accounts are disabled in production.</div><select aria-label="Local test account" className="field-input w-full mb-2" value={localKey} onChange={e => setLocalKey(e.target.value)} disabled={busy}>{localTestAccounts.map(account => <option key={account.key} value={account.key}>{account.label} — {account.blurb}</option>)}</select><button type="button" onClick={() => onLocalLogin(localKey)} disabled={busy} className="btn-secondary w-full justify-center">Log in as selected test account</button></div>}
      </div>
    </div>
  );
}
...
function normalizeHashTab(hash, isAdmin, isFullAdmin, isSuperadmin, isFellow) {
  ...
  // 'planners' (WA Staff) is open to the superadmin and to WA full-control
  // staff; 'roles' (role designations) stays superadmin-only.
  if (base === 'planners' && (isSuperadmin || isFullAdmin) && !isFellow) return base;
  if (base === 'roles' && isSuperadmin) return base;
  return home;
}

function MainApp({ auth, onLogout, onSwitchSystem = null }) {
  ...
  const [roles, setRoles] = useState(null);          // editable staff roles
  const [cityCodes, setCityCodes] = useState(null);  // placement-city call signs
  ...
  const [pillarTagFilter, setPillarTagFilter] = useState('all');
  ...
  const typeSaveTimer = useRef(null);
  const pillarTagSaveTimer = useRef(null);
  const modeSaveTimer = useRef(null);
  const rolesSaveTimer = useRef(null);
  const cityCodesSaveTimer = useRef(null);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        const r = await storage.get('wa14-session-types');
        setSessionTypes(r && r.value ? JSON.parse(r.value) : DEFAULT_SESSION_TYPES);
      }
      catch (e) { setSessionTypes(DEFAULT_SESSION_TYPES); }
      try {
        const r = await storage.get('wa14-pillar-tags');
        setPillarTags(r && r.value ? JSON.parse(r.value) : DEFAULT_PILLAR_TAGS);
      }
      catch (e) { setPillarTags(DEFAULT_PILLAR_TAGS); }
      try {
        const r = await storage.get('wa14-modes');
        setModes(r && r.value ? JSON.parse(r.value) : DEFAULT_MODES);
      }
      catch (e) { setModes(DEFAULT_MODES); }
      try {
        const r = await storage.get('wa14-roles');
        setRoles(r && r.value ? JSON.parse(r.value) : DEFAULT_STAFF_ROLES);
      }
      catch (e) { setRoles(DEFAULT_STAFF_ROLES); }
      try {
        const r = await storage.get('wa14-city-codes');
  ...
  const persistSessionTypes = debouncedPersist(setSessionTypes, typeSaveTimer, 'wa14-session-types');
  const persistPillarTags = debouncedPersist(setPillarTags, pillarTagSaveTimer, 'wa14-pillar-tags');
  const persistModes = debouncedPersist(setModes, modeSaveTimer, 'wa14-modes');
  const persistRoles = debouncedPersist(setRoles, rolesSaveTimer, 'wa14-roles');
  ...
  const addAccount = async (person, role) => {
    try {
      ...
      // The legacy login mirror carries the same systems + admin scope the
      // staff table holds, so role/access/system changes take effect at the
      // next sign-in even before the planner-table lookup is used.
      next.push({
        email: person.email, name: person.name, role, access,
        systems: person.systems || (role === 'superadmin' ? ['winter_academy', 'city'] : undefined),
        waAccess: person.waAccess, adminPanel: person.adminPanel,
      });
    ...
    <div className="wa14-main flex flex-1 min-h-screen">
      ...
      <div className="wa14-content flex-1 min-w-0">
        ...
        <div className="wa14-content-inner pt-5 px-6 pb-10">
          ...
          {tab === 'planners' && (isSuperadmin || (isFullAdmin && !isFellow)) && <PlannerPanel planners={planners} roles={roles} onChange={persistPlanners} onAccount={addAccount} showToast={showToast} isSuperadmin={isSuperadmin} selfEmail={auth.email} />}
...
function Sidebar({ tab, setTab, isAdmin, isFullAdmin, isSuperadmin, isFellow, openRequests }) {
  ...
  const tabs = [
    ...
    // WA Staff tab: superadmin, plus WA full-control staff (they manage roles
    // and access for others but can never touch the Superadmin role). Roles &
    // Codes stays superadmin-only: role designations are created there alone.
    ...((isSuperadmin || (isFullAdmin && !isFellow)) ? [{ id: 'planners', label: 'WA Staff', icon: IdentificationBadge }] : []),
    ...(isSuperadmin ? [{ id: 'roles', label: 'Roles & Codes', icon: LockKey }] : []),
  ];
  const go = (id) => (e) => {
...
function TopBar({ tab, isFullAdmin, auth, onLogout, onAdd, roles, onSwitchSystem = null }) {
  return (
    <div className="bg-wa-container/95 backdrop-blur-md border-b border-wa-border px-4 sm:px-6 flex items-center justify-between flex-wrap gap-3 sticky top-0 z-20 shadow-xs">
      ...
      <div className="flex items-center gap-3 py-2.5 flex-wrap">
        ...
        <div className="flex items-center gap-2.5 text-xs text-wa-muted border-l border-wa-borderdeep pl-3.5 min-w-0">
          ...
          {onSwitchSystem && (
            <button onClick={onSwitchSystem} title="Switch Training System" className="bg-transparent border border-wa-borderdeep cursor-pointer text-wa-text hover:text-wa-button transition-colors flex items-center gap-1.5 px-2.5 py-1.5 rounded-md hover:bg-wa-container2 text-xs font-semibold">
              <SquaresFour size={15} /> Switch system
            </button>
          )}
...
function layoutOverlapping(daySessions) {
  ...
  const flush = () => {
    ...
    cluster.forEach(item => {
      let col = colEnds.findIndex(endT => item.start >= endT);
      if (col === -1) { col = colEnds.length; colEnds.push(item.end); }
      else colEnds[col] = item.end;
      item.col = col;
    });
    const total = colEnds.length;
    cluster.forEach(item => { item.cols = total; result.push(item); });
    cluster = [];
    clusterEnd = -Infinity;
  };
  items.forEach(item => {
...
function RosterPanel({ roster, staff, cityCodes, onChange, onAccount, showToast }) {
  ...
  const addOne = (e) => {
    ...
    const fellow = { id: 'f' + Date.now(), name: name.trim(), email: em, afaGroup: afaGroup.trim(), track: track.toLowerCase(), grade: grade.trim(), placementCity: placementCity.trim(), cohort: defaultCohortForCreator({ role: 'afa' }), roomIds: [] };
    onChange([...roster, fellow]); onAccount(fellow, 'fellow');
    setName(''); setEmail(''); setAfaGroup(''); setTrack(''); setGrade(''); setPlacementCity(''); setError(''); showToast('Fellow added (cohort ' + fellow.cohort + ')');
  };
  const removeOne = (id) => { onChange(roster.filter(r => r.id !== id)); showToast('Fellow removed'); };
  const editOne = (fellow) => { const name = window.prompt('Fellow name', fellow.name); if (!name?.trim()) return; const email = window.prompt('Fellow email', fellow.email); if (!email?.trim()) return; const afa = window.prompt('AFA group', fellow.afaGroup || '') ?? (fellow.afaGroup || ''); const cohort = window.prompt('Cohort (year)', fellow.cohort || defaultCohortForCreator({ role: 'afa' })) ?? fellow.cohort; const next = { ...fellow, name: name.trim(), email: email.trim().toLowerCase(), afaGroup: afa.trim(), cohort: Number(cohort) || fellow.cohort, track: (window.prompt('Track: primary or secondary', fellow.track || '') || fellow.track || '').toLowerCase(), grade: window.prompt('Grade', fellow.grade || '') || fellow.grade || '', placementCity: window.prompt('Placement city', fellow.placementCity || '') || fellow.placementCity || '', roomIds: fellow.roomIds || [] }; onChange(roster.map(item => item.id === fellow.id ? next : item)); onAccount(next, 'fellow'); showToast('Fellow updated'); };
  const importBulk = () => {
    ...
    lines.forEach(line => {
      ...
      const fellow = { id: 'f' + Date.now() + added, name: nm, email: emLower, track: (parts[2] || '').toLowerCase(), grade: parts[3] || '', placementCity: parts[4] || '', afaGroup: parts[5] || '', roomIds: (parts[6] || '').split('|').filter(Boolean), cohort: Number(parts[7]) || defaultCohortForCreator({ role: 'afa' }) };
      ...
      <div style={{ background: '#003223', border: '1px solid #2A5C4B', borderRadius: 8, overflow: 'hidden', maxWidth: 520 }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
          ...
          <tbody>
            {sorted.map(r => (
              <tr key={r.id} style={{ borderBottom: '1px solid #1F4A3C' }}>
                <td style={{ padding: '8px 12px' }}>{r.name}<div style={{ fontSize: 11, color: '#9DB09D' }}>{r.email}</div></td><td style={{ padding: '8px 12px', color: '#D5E0D5' }}>{r.cohort ? r.cohort + ' · ' : ''}{r.track || '--'}{r.grade ? ' · ' + r.grade : ''}</td><td style={{ padding: '8px 12px', color: '#D5E0D5' }}>{r.placementCity ? <>{(cityCodes || []).find(c => c.city.toLowerCase() === r.placementCity.toLowerCase()) && <span style={{ ...callSignChipStyle, marginRight: 6 }}>{(cityCodes || []).find(c => c.city.toLowerCase() === r.placementCity.toLowerCase()).code}</span>}{r.placementCity}</> : '--'}{r.afaGroup ? ' · ' + r.afaGroup : ''}</td>
...
function PlannerPanel({ planners, roles, onChange, onAccount, showToast, isSuperadmin = false, selfEmail = '' }) {
  ...
  // Role options: 'superadmin' is offerable only to the superadmin; assigning
  // it needs a confirm and demoting the last one is blocked (see below).
  const roleOptions = isSuperadmin
    ? [...(roles || DEFAULT_STAFF_ROLES), { id: 'superadmin', label: 'Superadmin' }]
    : (roles || DEFAULT_STAFF_ROLES);

  const accessLabel = id => (STAFF_ACCESS.find(a => a.id === id) || {}).label || 'Resources only';

  // Any other remaining superadmin besides this address: another promoted
  // staff row, or the built-in superadmin account.
  const hasOtherSuperadmin = (excludeEmail) => (
    planners.some(p => p.role === 'superadmin' && String(p.email || '').toLowerCase() !== String(excludeEmail || '').toLowerCase())
    || (SUPERADMIN_EMAIL.toLowerCase() !== String(excludeEmail || '').toLowerCase())
  );
  ...
  const addOne = (e) => {
    ...
    if (role === 'superadmin' && !isSuperadmin) { setError('Only the Superadmin can assign the Superadmin role.'); return; }
    if (role === 'superadmin' && !window.confirm(name.trim() + ' will become a Superadmin with full control of everything. Continue?')) return;
    const cs = (callSign.trim() || callSignFromName(name)).toUpperCase();
    // New WA-only staff open the Winter Academy; City access can be added from
    // the City admin panel. A City staffer edited here keeps their systems.
    const planner = { id: 'p' + Date.now(), name: name.trim(), email: em, role, access, callSign: cs, group: AFA_ROLES.includes(role) ? group.trim() : '', systems: role === 'superadmin' ? ['winter_academy', 'city'] : undefined };
  ...
  const removeOne = (id) => {
    ...
    if (target?.role === 'superadmin' && !hasOtherSuperadmin(target.email)) { showToast('Cannot remove the last Superadmin'); return; }
    if (!window.confirm('Remove ' + (target?.name || 'this staff member') + ' from the staff list?')) return;
    onChange(planners.filter(p => p.id !== id)); showToast('Staff removed');
  };
  const [editingStaff, setEditingStaff] = useState(null);
  const [editName, setEditName] = useState('');
  const [editEmail, setEditEmail] = useState('');
  const [editRole, setEditRole] = useState('academy_lead');
  const [editAccess, setEditAccess] = useState('full');
  const [editGroup, setEditGroup] = useState('');
  const [editCallSign, setEditCallSign] = useState('');
  const [editError, setEditError] = useState('');

  const editOne = (planner) => {
    setEditName(planner.name || '');
    setEditEmail(planner.email || '');
    setEditRole(planner.role || 'academy_lead');
    setEditAccess(planner.access || 'full');
    setEditGroup(planner.group || '');
    setEditCallSign(planner.callSign || '');
    setEditError('');
    setEditingStaff(planner.id);
  };
  ...
  const saveEdit = (e) => {
    ...
    if (editRole === 'superadmin' && !isSuperadmin) { setEditError('Only the Superadmin can assign the Superadmin role.'); return; }
    const wasSuperadmin = editingStaff?.role === 'superadmin';
    if (wasSuperadmin && editRole !== 'superadmin' && !hasOtherSuperadmin(editingStaff.email)) { setEditError('Cannot demote the last Superadmin. Promote someone else first.'); return; }
    if (editRole === 'superadmin' && !wasSuperadmin && !window.confirm(editName.trim() + ' will become a Superadmin with full control of everything. Continue?')) return;
    ...
    const updated = {
      ...
      callSign: cs,
      systems: editRole === 'superadmin' ? ['winter_academy', 'city'] : editingStaff.systems,
    };
    ...
    <div>
      ...
      <div style={{ background: '#003223', border: '1px solid #2A5C4B', borderRadius: 8, padding: '12px 16px', marginBottom: 16, maxWidth: 520, display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11.5, color: '#9DB09D' }}>Superadmin · built-in and permanent. Only a Superadmin can promote another Superadmin; the last one can never be removed or demoted · can also be selected as a facilitator</div>
        </div>
      </div>
      <div style={{ background: '#003223', border: '1px solid #2A5C4B', borderRadius: 8, padding: 18, marginBottom: 20, maxWidth: 560 }}>
        <form onSubmit={addOne} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '1 1 150px' }}><div style={{ fontSize: 12, color: '#D5E0D5', fontWeight: 600, marginBottom: 5 }}>Role</div><select className={inputStyle} value={role} onChange={e => setRole(e.target.value)}>{roleOptions.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}</select></div>
                ...
                <tr key={p.id} style={{ borderBottom: '1px solid #1F4A3C' }}>
                  <td style={{ padding: '12px 12px', colSpan: 5 }}>
                    <form onSubmit={saveEdit} style={{ display: 'flex', gap: 8, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                      ...
                      <div style={{ flex: '1 1 140px' }}><div style={{ fontSize: 12, color: '#D5E0D5', fontWeight: 600, marginBottom: 5 }}>Role</div><select className={inputStyle} value={editRole} onChange={e => setEditRole(e.target.value)}>{roleOptions.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}</select></div>
