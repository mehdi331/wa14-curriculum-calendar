...
import { Plus, X, Download, ArrowCounterClockwise as RotateCcw, Users, Clock, Calendar as CalendarIcon, Table as TableIcon, ChartBar as BarChart3, Link as LinkIcon, SignOut as LogOut, UserPlus, Trash as Trash2, ShieldCheck, ChatCircle as MessageSquare, PaperPlaneTilt as Send, DoorOpen } from '@phosphor-icons/react';
...
export default function App(){
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      setAuthError(error.code === 'auth/popup-closed-by-user' ? '' : 'Google sign-in could not be completed. Please try again.');
    }
  };
  ...
  if (!auth) return <LoginGate onLogin={handleLogin} error={authError} />;
...
async function resolveRole(user){
  ...
  if (em === SUPERADMIN_EMAIL) return { ok:true, email:em, role:'superadmin', name:user.displayName || 'Superadmin', uid:user.uid };

  if (STAFF_EMAIL_RE.test(em)) {
    const match = accounts.find(a => a.email.toLowerCase()===em);
    return { ok:true, email:em, role:match?.role || 'staff', name:match?.name || user.displayName || em.split('@')[0], uid:user.uid };
  }
  ...
  if (FELLOW_EMAIL_RE.test(em)) {
    try {
      ...
      if (match) {
        return { ok:true, email:em, role:'fellow', name:match.name, fellowId:match.id, uid:user.uid };
      }
    } catch (e) { return { ok:false, error:'Could not verify right now — please try again.' }; }
  ...
  return (
    <div className="wa14-app" style={{fontFamily:FONT, minHeight:'480px', display:'flex', alignItems:'center', justifyContent:'center', background:'#EFF3F4'}}>
      <div style={{background:'#fff', border:'1px solid #DDE2E6', borderRadius:10, padding:32, width:340, maxWidth:'88vw'}}>
        ...
        <button type="button" onClick={onLogin} style={{...btnPrimary, width:'100%', justifyContent:'center', padding:'10px', marginTop:6}}>Continue with Google</button>
      </div>
    </div>
  );
}
...
function MainApp({ auth, onLogout }){
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setRoster(fellows.map(({ pin, ...fellow }) => fellow));
      }
      catch (e) { setRoster([]); }
      try {
  ...
  const addAccount = async (person, role) => {
    try {
      ...
      next.push({email:person.email, name:person.name, role});
  ...
  return (
    <div className="wa14-app" style={{fontFamily:FONT, background:'#EFF3F4', minHeight:'600px', color:'#1B2733'}}>
      <TopBar
...
function RosterPanel({ roster, onChange, onAccount, showToast }){
  ...
  const addOne = (e) => {
    ...
    const fellow = { id: 'f'+Date.now(), name: name.trim(), email: em };
  ...
  const importBulk = () => {
    ...
    lines.forEach(line => {
      ...
      const fellow = { id: 'f'+Date.now()+added, name: nm, email: emLower };
...
function PlannerPanel({ planners, onChange, onAccount, showToast }){
  ...
  const addOne = (e) => {
    ...
    const planner = { id:'p'+Date.now(), name:name.trim(), email:em, role };
  ...
  return (
    <div>
      ...
      <div style={{background:'#fff', border:'1px solid #DDE2E6', borderRadius:8, overflow:'hidden', maxWidth:520}}>
        <table style={{width:'100%', borderCollapse:'collapse', fontSize:12.5}}>
          <thead><tr style={{background:'#F7F8F9', textAlign:'left'}}><th style={{padding:'9px 12px', fontWeight:600, color:'#5B6672', borderBottom:'1px solid #DDE2E6'}}>Name</th><th style={{padding:'9px 12px', fontWeight:600, color:'#5B6672', borderBottom:'1px solid #DDE2E6'}}>Role</th><th style={{padding:'9px 12px', borderBottom:'1px solid #DDE2E6'}}></th></tr></thead>
          <tbody>
            {sorted.map(p => (
              <tr key={p.id} style={{borderBottom:'1px solid #EEF0F2'}}>
                <td style={{padding:'8px 12px'}}>{p.name}<div style={{fontSize:11,color:'#8A96A3'}}>{p.email}</div></td><td style={{padding:'8px 12px', color:'#5B6672'}}>{ROLE_LABEL[p.role]||'Planning team'}</td>
